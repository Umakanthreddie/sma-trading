"""
config.py — SMA Trader Configuration
Edit this file to customize your watchlist, MA periods, and alert settings.

BROKER is back to "paper" — the app's own built-in simulator. The
unofficial `webull` library's login is currently broken against Webull's
servers (confirmed: it gets back an empty response), so scripted access
to Webull's paper account isn't reliable right now. This app is for
generating signals/recommendations for you to review and place manually
in the Webull app yourself.
"""
